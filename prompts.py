...
few_shot_prompt_table = """
    ...
    - High (Label): ${high_label} (This is the highest price recorded after the chart pattern)
    - Low (Label): ${low_label} (This is the lowest price recorded after the chart pattern)
    - Stock Data: ${stock_data} (This is tabular data from stock formation)
    - Chart: See the image below.
    """
...
You have now learned from the previous examples provided in the earlier step. Based on your knowledge from those few-shot examples, 
analyze the following chart and its indicators to predict the stock price movement for the next period.
...
output just expected High price in following output, no yapping:
...
system_prompt = (
    "You are an advanced AI stock analyst with deep expertise in financial markets."
)
